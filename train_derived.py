...
from models_search.vit_face import ViT_face
...
def main():
    args = cfg.parse_args()
    wandb.init(project="TransGAN", name="celeba_256_w_recon", config=args)
...
#                 raise NotImplementedError('{} unknown inital type'.format(args.init_type))
    ...
    if not torch.cuda.is_available():
        ...
        if args.gpu is not None:
            ...
            vit_encoder = ViT_face(image_size=112,
                                   patch_size=8,
                                   loss_type="CosFace",
                                   GPU_ID=0,
                                   num_class=93431,
                                   ...
                                   mlp_dim=2048,
                                   dropout=0.1,
                                   emb_dropout=0.1,).cuda(args.gpu)
            vit_encoder.load_state_dict(torch.load(
                "weights/vit_face.pth", map_location="cuda"))
            vit_encoder.eval()
            ...
            vit_encoder = torch.nn.parallel.DistributedDataParallel(
                vit_encoder, device_ids=[args.gpu], find_unused_parameters=True)
        else:
            gen_net.cuda()
            dis_net.cuda()
...
#         gen_avg_param = list(p.cuda().to(f"cuda:{args.gpu}") for p in gen_avg_param)
    ...
    for epoch in range(int(start_epoch), int(args.max_epoch)):
        ...
        train(args, gen_net, dis_net, vit_encoder, gen_optimizer, dis_optimizer, gen_avg_param, train_loader, epoch, writer_dict, fixed_z,
              lr_schedulers)
        ...
        if epoch and epoch % args.val_freq == 0 or epoch == int(args.max_epoch)-1:
            ...
            inception_score, fid_score = validate(
                args, fixed_z, fid_stat, epoch, gen_net, writer_dict)
            if args.rank == 0:
                logger.info(
