...
def train(args, gen_net: nn.Module, dis_net: nn.Module, vit_encoder, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
    ...
    for iter_idx, (imgs, _) in enumerate(tqdm(train_loader)):
        ...
        #     0, 1, (imgs.shape[0], args.latent_dim))).cuda(args.gpu, non_blocking=True)
        z = vit_encoder(F.interpolate(
            real_imgs, size=(112, 112), mode='bilinear').cuda()).detach()
        z = F.normalize(z, p=2, dim=1).to('cuda')
        ...
        if args.loss == 'hinge':
            ...
            real_label = torch.full(
                (imgs.shape[0],), 1., dtype=torch.float, device=real_imgs.get_device())
            fake_label = torch.full(
                (imgs.shape[0],), 0., dtype=torch.float, device=real_imgs.get_device())
            ...
            if isinstance(fake_validity, list):
                ...
                for real_validity_item, fake_validity_item in zip(real_validity, fake_validity):
                    real_label = torch.full(
                        (real_validity_item.shape[0], real_validity_item.shape[1]), 1., dtype=torch.float, device=real_imgs.get_device())
                    fake_label = torch.full(
                        (real_validity_item.shape[0], real_validity_item.shape[1]), 0., dtype=torch.float, device=real_imgs.get_device())
                    d_real_loss = nn.MSELoss()(real_validity_item, real_label)
                    d_fake_loss = nn.MSELoss()(fake_validity_item, fake_label)
                    d_loss += d_real_loss + d_fake_loss
            else:
                real_label = torch.full(
                    (real_validity.shape[0], real_validity.shape[1]), 1., dtype=torch.float, device=real_imgs.get_device())
                fake_label = torch.full(
                    (real_validity.shape[0], real_validity.shape[1]), 0., dtype=torch.float, device=real_imgs.get_device())
        ...
        if (iter_idx + 1) % args.accumulated_times == 0:
            ...
            wandb.log({'d/loss': d_loss.item()},
                      step=global_steps) if args.rank == 0 else 0
        # -----------------
        #  Train Generator
        # -----------------
        if global_steps % (args.n_critic * args.accumulated_times) == 0:
            ...
            for accumulated_idx in range(args.g_accumulated_times):
                ...
                gen_z = vit_encoder(F.interpolate(
                    real_imgs, size=(112, 112), mode='bilinear').cuda()).detach()
                gen_z = F.normalize(gen_z, p=2, dim=1).to('cuda')
                ...
                if args.loss == "standard":
                    real_label = torch.full(
                        (args.gen_batch_size,), 1., dtype=torch.float, device=real_imgs.get_device())
                    fake_validity = nn.Sigmoid()(fake_validity.view(-1))
                ...
                if args.loss == "lsgan":
                    if isinstance(fake_validity, list):
                        ...
                        for fake_validity_item in fake_validity:
                            real_label = torch.full(
                                (fake_validity_item.shape[0], fake_validity_item.shape[1]), 1., dtype=torch.float, device=real_imgs.get_device())
                            g_loss += nn.MSELoss()(fake_validity_item, real_label)
                    else:
                        real_label = torch.full(
                            (fake_validity.shape[0], fake_validity.shape[1]), 1., dtype=torch.float, device=real_imgs.get_device())
                        # fake_validity = nn.Sigmoid()(fake_validity.view(-1))
                        g_loss = nn.MSELoss()(fake_validity, real_label)
                elif args.loss == 'wgangp-mode':
                    fake_image1, fake_image2 = gen_imgs[:args.gen_batch_size //
                                                        2], gen_imgs[args.gen_batch_size//2:]
                    z_random1, z_random2 = gen_z[:args.gen_batch_size //
                                                 2], gen_z[args.gen_batch_size//2:]
                ...
                reconstruction_loss = F.mse_loss(
                    real_imgs, gen_imgs) * 10. / float(args.g_accumulated_times)
            ...
            if args.ema_warmup != 0:
                ...
                ema_beta = 0.5 ** (float(args.dis_batch_size *
                                   args.world_size) / max(ema_nimg, 1e-8))
            else:
                ema_beta = args.ema
            ...
            writer.add_scalar('g_loss', g_loss.item(),
                              global_steps) if args.rank == 0 else 0
            wandb.log({'g/loss': g_loss.item()},
                      step=global_steps) if args.rank == 0 else 0
            wandb.log({'g/rec': reconstruction_loss.item()},
                      step=global_steps) if args.rank == 0 else 0
            gen_step += 1
...
#             save_image(sample_imgs, f'sampled_images_{args.exp_name}.jpg', nrow=4, normalize=True, scale_each=True)
            ...
            wandb.log({"examples": [wandb.Image(
                im) for im in sample_imgs.cpu().detach()]}, step=global_steps)
            tqdm.write(
...
#     mean, std = 0, 0
    ...
    if args.rank == 0:
        fid_score = get_fid(args, fid_stat, epoch, gen_net, args.num_eval_imgs,
                            args.gen_batch_size, args.eval_batch_size, writer_dict=writer_dict, cls_idx=None)
    else:
        fid_score = 10000
    # fid_score = 10000
...
#     writer.add_image('sampled_images', img_grid, global_steps)
    if args.rank == 0:
        ...
        wandb.log({"IS/mean": mean, "IS/std": std,
                  "FID": fid_score}, step=global_steps)
...
def save_samples(args, fixed_z, fid_stat, epoch, gen_net: nn.Module, writer_dict, clean_dir=True):
    ...
    with torch.no_grad():
        ...
        save_image(
            sample_imgs, f'./samples/{args.exp_name}/sampled_images_{epoch}.png', nrow=10, normalize=True, scale_each=True)
    return 0
...
def get_topk_arch_hidden(args, controller, gen_net, prev_archs, prev_hiddens):
    ...
    topk_arch_idx_perf = sorted(arch_idx_perf_table.items(
    ), key=operator.itemgetter(1))[::-1][:args.topk]
    topk_archs = []
    topk_hxs = []
    topk_cxs = []
